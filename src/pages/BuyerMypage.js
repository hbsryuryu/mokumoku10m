...
const BuyerMypage = () => {
	...
	return (
		<>
			<CommonHeader />
			<p className="g-titlebar">マイページ</p>
			{pageState === 0 && state.userData.is_guest && (
				<>
					<p className="c9-p12">新規会員登録</p>
					<button className="c9-btn1 g-btn1 g-btn-clr1" onClick={() => navigate(myUrl("b_u_regi"))}>
						新規会員登録
					</button>
					<p className="c9-p12">ログイン</p>
					<button className="c9-btn1 g-btn1 g-btn-clr2" onClick={() => setPageState(1)}>
						ログイン
					</button>
				</>
			)}
			{pageState === 0 && !state.userData.is_guest && (
				<>
					<div className="c9-div12">
						<img className="c9-div12img" src={baseUrl + state.userData.campany_image_url + imgReload} alt="" />
						<p className="c9-div12p1">{state.userData.nick_name}</p>
					</div>

					<button className="g-btn1 g-btn-clr1 g-btn-sp1" onClick={() => navigate(myUrl("b_i_recv_l"))}>
						購入した商品
					</button>
					<button
			...
			{pageState === 2 && (
				<>
					...
					{/* <p>ランク：{state.userData.userGrade}</p> */}
					{/* <p>取引数：{state.userData.tradeCount}</p> */}
